...
@section('title', 'Ajouter un examen — ' . $hospital->name)
...
<h1 class="mb-6 mt-1 text-xl font-semibold text-slate-900">Ajouter un examen</h1>

<div class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6">
    <form method="POST" action="{{ route('admin.hospitals.exam-templates.store', $hospital) }}" class="space-y-6">
        @csrf
